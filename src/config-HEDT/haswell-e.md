...
Принудительно перезаписывает переменные NVRAM, обратите внимание, что `Add` **не переписывает** значения присутствующие в NVRAM, поэтому значения вроде `boot-args` следует оставить в покое. Из-за проблем с NVRAM на X99, мы изменим следующее:
...
На примере этого Ivy Bridge-E, мы выберим iMacPro1,1 SMBIOS.
