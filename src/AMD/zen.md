...
| **[SSDT-CPUR](https://github.com/naveenkrdy/Misc/blob/master/SSDTs/SSDT-CPUR.dsl)** | Исправляет определение процессора на материнскиз платах B550 и A520, **не используйте**, если у вас нет системы на AMD B550 или A520. Вы можете найти готовый вариант здесь: [SSDT-CPUR.aml](https://github.com/dortania/Getting-Started-With-ACPI/blob/master/extra-files/compiled/SSDT-CPUR.aml) |
...
* **SetupVirtualMap**: YES
  ...
  * Платы B550, A520 и TRx40 должны отключить этот квирк
* **SyncRuntimePermissions**: YES
...
* **DisableIoMapper**: NO
  * AMD не поддерживает DMAR или VT-D, так что не актуально
...
Forcibly rewrites NVRAM variables, do note that `Add` **will not overwrite** values already present in NVRAM so values like `boot-args` should be left alone. For us, we'll be changing the following: Принудительно перезаписывает переменные NVRAM, обратите внимание, что `Add` **не переписывает** значения присутствующие в NVRAM, поэтому значения вроде `boot-args` следует оставить в покое. Мы изменим следующее:
...
* MacPro7,1: AMD RX Polaris и новее(Обратите внимание, что для MacPro7,1 минимальная версия - Catalina)
...
* Serial/COM Port
* Parallel Port
* Compatibility Support Module (CSM)(**Должно быть выключено, ошибки GPU, такие как `gIO` - обычное явление, когда эта опция включена**)
